...
export default function Dashboard() {
  ...
  const [ticketsCounts, setTicketsCounts] = useState({ openticketscount: 0, closedticketscount: 0 })
  ...
  const fetchPassengers = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/tickets/status/alltickets`)
  ...
  const handleSave = async (id) => {
    setSaveLoading(true)
    try {
      await axios.put(`${BASE_URL}/tickets/${id}/updateticketdetails`, editForm)
      setEditingId(null)
      fetchPassengers()
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${BASE_URL}/tickets/${id}/deleteticketdetails`)
  ...
  const totalSeats = 40
  const bookedSeats = ticketsCounts.closedticketscount ?? 0
  const availableSeats = ticketsCounts.openticketscount ?? 0
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <table className="w-full text-sm border border-black-200">
            ...
            <tbody className="divide-y divide-black-300">
              {passengers.map((p) => (
                <tr key={p._id} className={`hover:bg-gray-50 transition-colors ${editingId === p._id ? 'bg-blue-50/30' : ''}`}>
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.4)' }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4 shadow-xl">
            ...
            <p className="text-sm text-gray-500 mb-5">This will permanently remove the reservation. This action cannot be undone.</p>
